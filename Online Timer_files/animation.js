...
$(document).ready(function() {
  ...
  $stage.on('click', function() {   
    if (typeof te_html !== 'undefined') {
      te_html.click('clickTAG', window.clickTag);
    } else {
  ...
  for (var i = 0; i < 100; i++) {
    tweenBox1(getBox(i), getRandom(0, 3), i);
  }

  for (var j = 0; j < 100; j++) {
    tweenBox1(getBox(i+j), getRandom(4, 7), i+j);
  }
  
  for (var l = 0; l < 200; l++) {
    tweenBox1(getBox(i+j+l), getRandom(7, 10), i+j+l);
  }
